...
export default function PartnerMenuUploadPage() {
  ...
  useEffect(() => {
    ...
    const loadSession = async () => {
      try {
        ...
        console.error('[partner/menu-upload] failed to load partner session', loadError)
        router.replace('/auth?mode=partner')
      } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
      <Head>
        <title>Upload menu · {partner.restaurant_name || 'Your restaurant'}</title>
      </Head>
...
PartnerMenuUploadPage.getLayout = function getLayout(page) {
